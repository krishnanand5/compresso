...
describe('resolveGptProfile style overrides', () => {
  it('merges every render knob into the selected model profile', () => {
    const prev = process.env.COMPRESSO_GPT_PROFILES;
    try {
      process.env.COMPRESSO_GPT_PROFILES = JSON.stringify({
        'gpt-5.6-sol': {
          stripCols: 100,
          style: {
            font: 'spleen-5x8',
      ...
      if (prev === undefined) delete process.env.COMPRESSO_GPT_PROFILES;
      else process.env.COMPRESSO_GPT_PROFILES = prev;
    }
  });
});
