...
export interface Env {
  ...
  COMPRESSO_TRACK?: string;
  /** Shared secret callers must present via the `x-compresso-secret` header
   ...
   *    npx wrangler secret put COMPRESSO_WORKER_SECRET */
...
export default {
  async fetch(req: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    if (env.ANTHROPIC_API_KEY || env.OPENAI_API_KEY) {
      if (!env.COMPRESSO_WORKER_SECRET) {
        return new Response(
          JSON.stringify({
            error:
              'refusing to proxy: an API key override is configured but COMPRESSO_WORKER_SECRET is not, ' +
              'which would let anyone who finds this URL spend the configured key. ' +
              'Run `npx wrangler secret put COMPRESSO_WORKER_SECRET` and send the value as the x-compresso-secret header.',
          }),
          { status: 503, headers: { 'content-type': 'application/json' } },
        );
      }
      const presented = req.headers.get('x-compresso-secret') ?? '';
      if (!(await secretsMatch(presented, env.COMPRESSO_WORKER_SECRET))) {
        return new Response(
          JSON.stringify({ error: 'missing or invalid x-compresso-secret header' }),
          { status: 401, headers: { 'content-type': 'application/json' } },
        );
      }
      // Don't forward the shared secret upstream.
      req = new Request(req);
      req.headers.delete('x-compresso-secret');
    }
    ...
    const trackingOn = truthy(env.COMPRESSO_TRACK, true);
    ...
    const sharedUpstream = env.COMPRESSO_UPSTREAM;
