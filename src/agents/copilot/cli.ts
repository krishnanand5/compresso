...
function parseArgv(): CopilotCliOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === '--model' || a === '-m') { opts.model = args[++i]; continue; }
    if (a === '--system' || a === '-s') { opts.systemPrompt = args[++i]; continue; }
...
function printStats(stats: { compressedCount: number; origTokensTotal: number; imageTokensTotal: number; tokenSavingsPct: number } | null): void {
  if (!stats || stats.compressedCount === 0) { console.log('  (no compression data yet)'); return; }
  console.log('');
...
async function runREPL(opts: CopilotCliOptions): Promise<void> {
  ...
  let session;
  try {
    session = await createSession({ model: opts.model, systemPrompt: opts.systemPrompt, compress: true, logLevel: 'error', skipCustomInstructions: false });
  } catch (err) {
  ...
  for await (const line of rl) {
    ...
    if (trimmed === '/exit') { cleanup(); break; }
    if (trimmed === '/stats') { printStats(session?.compressionStats ?? null); rl.prompt(); continue; }
    if (trimmed === '/clear') { console.clear(); rl.prompt(); continue; }
    try {
...
async function runSingleShot(opts: CopilotCliOptions): Promise<void> {
  ...
  if (!opts.quiet) {
    const stats = session.compressionStats;
    if (stats && stats.compressedCount > 0) { console.log(''); printStats(stats); }
  }
  await session.close();
}
...
async function main(): Promise<void> {
  ...
  if (opts.prompt) { await runSingleShot(opts); }
  else { await runREPL(opts); }
}
