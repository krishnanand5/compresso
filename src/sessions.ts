 ...
 * better-than-good-enough session key without coupling compresso to Claude
 ...
 *
 * - `~/.compresso/events.jsonl` — append-only JSONL written by FileTracker
 * - `~/.compresso/4xx-bodies/${iso-ts}-${sha8}.json.gz` — gzipped failure
...
export function defaultPaths(): SessionsPaths {
  ...
  const eventsFile =
    process.env.COMPRESSO_LOG ?? path.join(home, '.compresso', 'events.jsonl');
 ...
 *  lines are silently dropped — matches `compresso stats` behavior. */
 ...
 * missing, returns an empty map without throwing — compresso must keep
