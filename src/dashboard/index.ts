 ...
 * every request — compresso doesn't have a query layer, but a 1.5 MB JSONL
 ...
 *   GET  /api/stats.json             → full-history aggregate (formerly `compresso stats`)
 *
...
import { registerStatsProvider, getStatsProvider } from './api/agents.js';
 ...
 *      state: compresso images the cached prefix in place (moves the caller's
...
interface SessionTotals {
  ...
  // Raw output tokens (the model's reply). compresso does NOT compress output, so
...
interface Totals {
   ...
   *  compresso or we can't measure what it would have cost, so the
   ...
   *  saved/actual exceed 100% because compresso shrunk the actual to
   *  near zero. */
 ...
 *  compresso actually paid — not a count_tokens probe. No per-session warmth state:
...
export class DashboardState {
  ...
  update(ev: ProxyEvent): void {
    ...
    if (gpt) {
      ...
      // compresso only reduces input by imaging the static slab. An UNCOMPRESSED
      ...
      // Cache-aware, server-observed baseline. INVARIANT: compresso is credited ONLY
    ...
    // denominator for "did compresso move my real bill". Measured rows
    // contribute their cache-aware baseline (what the unproxied path
    // would have billed); unmeasured/probe-failed/passthrough rows
    // contribute their actual input (compresso either didn't run or we
  ...
  serveStats(): Response {
    ...
    // answers "did compresso help on the rows where it ran" instead of
    // "did compresso move my real bill". The first is a cherry-pick.
    ...
    // paid with no compresso. Bounded ratio at 100%; a single cold-miss
    ...
    const payload = {
      ...
      // ask "is compresso helping". Negative when flap-pollution from
   ...
   *  runtime compress scope. In-memory only; restart resets to the COMPRESSO_MODELS
  ...
  /** GET /api/agents/<id>/stats.json */
  async serveAgentStats(agentId: string): Promise<Response> {
    const fn = getStatsProvider(agentId);
    if (!fn) return jsonResponse({ error: `unknown agent: ${agentId}` }, 404);
    const stats = await fn({ eventsFile: this.paths?.eventsFile });
    if (!stats) return jsonResponse({ error: `no stats for agent: ${agentId}` }, 404);
...
registerStatsProvider('copilot', async (opts) => {
  const copilotFile = opts?.eventsFile
    ...
    : undefined;
  if (!copilotFile || !fs.existsSync(copilotFile)) return undefined;
  const agg = await readCopilotEventsFile(copilotFile);
...
export type DashboardRoute =
  ...
  | { kind: 'agent-stats'; agentId: string }; // /api/agents/<id>/stats.json
...
export function dashboardPath(pathname: string): DashboardRoute | null {
  ...
  const agentsMatch = /^\/api\/agents\/([^/]+)\/stats\.json$/.exec(pathname);
  if (agentsMatch) return { kind: 'agent-stats', agentId: agentsMatch[1]! };
