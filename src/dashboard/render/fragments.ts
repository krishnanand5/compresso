...
export function renderModelsFragment(
  ...
  return (
    ...
    claudeChips +
    `<span class="hint">everything else is sent as normal text · runtime only · persist with COMPRESSO_MODELS</span>${moot}` +
    `</div>` +
    `<div class="models">` +
    `<span class="models-label">Image Grok models</span>` +
    grokChips +
    otherChips +
    `<span class="hint">opt-in only · OpenAI Responses path · set COMPRESSO_MODELS to persist</span>${moot}` +
    `</div>` +
    `<div class="models">` +
    `<span class="models-label">Image GPT models</span>` +
    gptChips +
    `<span class="hint">imaging only, no Anthropic cache_control · one scope for all families · set COMPRESSO_MODELS (CSV of bases, or off) to persist</span>${moot}` +
    `</div>`
  );
}
...
// purpose ("lifeweight"): it answers "did compresso move my real, cache-discounted
...
export function renderSessionSummaryFragment(s: StatsPayload): string {
  ...
  // Input-only: compresso never touches output, so lumping it in just dampened the %.
...
export function renderHeaderFragment(s: StatsPayload, port: number): string {
  ...
  const costTile = splitReady
    ? statTile(
        'Cost per request',
        `$${cAvg.toFixed(4)}`,
        `vs $${pAvg.toFixed(4)} without compresso`,
        cAvg <= pAvg ? 'pos' : 'neg',
...
export function renderContextMapFragment(
  ...
  // compresso effect, so cold rows price both paths cold.
  ...
  return (
    ...
    `<div class="split-note">compresso can misread exact values inside images — treat these as gist, not byte-exact.</div>` +
    `</div>` +
...
export function renderLatestFragment(inp: LatestFragmentInput): string {
  ...
  if (pin != null && pinnedEvicted) {
    ...
    main = `<div class="empty-note">No images yet — they appear the instant compresso compresses a request.</div>`;
  }
...
<title>compresso — live dashboard</title>
...
<header class="topbar">
  <div class="brand">
    ...
    <div>
      <div class="wordmark">compresso</div>
