/**
 * compresso proxy as a single Web-standard fetch handler.
...
/** Parse COMPRESSO_GATEWAY_HEADERS — JSON object or `k=v;k2=v2`. */
  ...
  return async function handle(req: Request): Promise<Response> {
    ...
    if (isOpenAIChat || isOpenAIResponses || isOpenCodeTransformable) {
      ...
      try {
        ...
        return new Response(JSON.stringify({ error: 'compresso transform failed' }), {
          status: 502,
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'compresso upstream unreachable' }), {
        status: 502,
        headers: { 'content-type': 'application/json' },
      });
    }
