 ...
 *  compresso itself (rendered dumps, OCR, this very transcript). {@link neutralizeSentinel}
...
export interface RenderDensePagesOptions {
   ...
   *  that pre-reflow (the proxy's maybeReflow / history lockstep) pass false; `compresso export`
 ...
 * `renderTextToImages` (library.ts → `compresso export`) AND the proxy's `textToImageBlocks`
