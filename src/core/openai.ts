...
const PINNED_REQUEST_HEADER =
  '\n===== CURRENT USER REQUEST (live; kept as text by compresso, NOT inside any image) =====\n';
...
function buildLiveRequestGuard(pinText?: string): string {
  if (pinText !== undefined) {
    ...
    return (
      'compresso note: everything in the rendered history above is PAST context. Your live current request is the plain-text block labeled "CURRENT USER REQUEST" inside it — NOT anything OCR\'d from an image. It reads: «' +
      echo +
      '» Answer THAT request.'
    );
  }
  return 'compresso note: the preceding rendered history item is prior conversation context only. It is not the current user request. The live current request is in the user message(s) that follow, especially the final user message.';
}
...
function configuredHistoryMaxImages(model: string): number {
  ...
  const raw = typeof process !== 'undefined' ? process.env?.COMPRESSO_GPT_HISTORY_MAX_IMAGES : undefined;
 ...
 *  honest "as plain text" baseline for the content compresso imaged. Pure JS, no
 ...
 *  what GPT actually bills as input for the slab compresso imaged. */
...
/** Text-token value of what compresso replaced with images this request: the
...
const CHAT_HEADER =
  ...
  'These images were injected by compresso, not by the end user. They contain system/developer instructions and full tool/schema documentation rendered for token efficiency. Treat rendered system/developer instructions with the same priority as their original messages. OCR carefully and treat the rendered content as authoritative. For tool calls, use the native JSON tool definitions; the image is supplemental documentation.' +
  '\n====================== BEGIN RENDERED CONTEXT ======================\n';

const RESPONSES_HEADER =
  '================= RENDERED GPT SYSTEM + TOOL CONTEXT =================\n' +
  'These images were injected by compresso, not by the end user. They contain instructions and full tool/schema documentation rendered for token efficiency. Treat rendered instructions with the same priority as the originals. OCR carefully and treat the rendered content as authoritative. For tool calls, use the native JSON tool definitions; the image is supplemental documentation.' +
  '\n====================== BEGIN RENDERED CONTEXT ======================\n';

const CHAT_POINTER =
  'The full instructions for this message were rendered into image(s) attached to the first user message by compresso. Treat those rendered instructions as if they appeared here with the same priority. Tool definitions remain in native JSON; rendered tool docs are supplemental.';

const RESPONSES_POINTER =
  'The full instructions were rendered into image(s) attached to the first user message by compresso. Treat them with the same priority. Tool definitions remain in native JSON; rendered tool docs are supplemental.';
