 ...
 *
 * Retune without a code change via the COMPRESSO_GPT_PROFILES env var (JSON map of
 ...
 *
 *   COMPRESSO_GPT_PROFILES='{"gpt-5.6-sol":{"vision":{"regime":"patch","multiplier":1,"patchCap":12000},"stripCols":120,"maxHeightPx":1900}}'
 *   COMPRESSO_GPT_PROFILES='{"gpt-5.6-sol":{"style":{"grid":true,"gridCols":4}}}'
 */
...
// --- env override (COMPRESSO_GPT_PROFILES) -----------------------------------
...
function envProfiles(): Map<string, GptModelProfile> {
  const raw = (typeof process !== 'undefined' && process.env && process.env.COMPRESSO_GPT_PROFILES) || '';
  if (raw !== envRaw) {
    envRaw = raw;
