/**
 * Core logic for `compresso export` — renders a source text to PNG pages, extracts
...
// consumers import (compresso/transform → renderTextToImages), not the
// internal leaf renderer.
import { renderTextToImages } from './library.js';
import { estimateImageCount, REPORT_CHARS_PER_TOKEN } from './utils.js';
...
export function buildPromptText(
  ...
  return (
    `These ${pageCount} image${pageCount !== 1 ? 's' : ''} contain source code/text rendered as PNG pages by compresso.\n\n` +
    fileSection +
...
export async function runExportCore(
  ...
  // SDK consumers (compresso/transform).
