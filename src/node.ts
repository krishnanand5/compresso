...
function parseCli(argv: string[]): RuntimeConfig {
  ...
  // error — there is exactly ONE way to run compresso and the dashboard
...
// inside its own run-script env, so for `npx compresso` or a global bin it is
...
class FileTracker implements Tracker {
  ...
  emit(ev: TrackEvent): void {
    ...
    try {
      ...
      if (!this.brokenLogged) {
        console.error(
          `[compresso] FileTracker write failed: ${(err as Error).message}`,
        );
        this.brokenLogged = true;
      }
    }
...
async function collectSource(opts: ExportParsed): Promise<[string, string[]]> {
  ...
  if (files.length === 0) {
    console.warn('[compresso export] no files collected');
  }
...
async function runExport(argv: string[]): Promise<void> {
  ...
  // Unique output dir: <out>/compresso-export-XXXXXX/. mkdtemp guarantees a fresh, random
...
async function main(): Promise<void> {
  ...
  // Debug aid: when COMPRESSO_DUMP_DIR is set, persist every rendered PNG this
  ...
  // Note: the COMPRESSO_DISABLE arm renders nothing, so only the compress proxy
  // produces files here.
  ...
  // below is ONLY a kill switch (COMPRESSO_DISABLE / dashboard toggle →
  ...
  const config: ProxyConfig = {
    ...
    transform: () => {
      // A/B harness: COMPRESSO_DISABLE=1 forces passthrough (compress=false) for the
      // whole process, so the "normal" arm can be scripted on its own port while
      // still logging real usage + count_tokens baselines to its own COMPRESSO_LOG.
    ...
    onRequest: async (e) => {
      ...
      dashboard.update(e);
      // Debug: persist this request's rendered PNGs (see COMPRESSO_DUMP_DIR above).
      ...
      // Persistent JSONL event for offline analysis (compresso stats etc.).
  ...
  const shutdown = (sig: string) => {
    if (shuttingDown) {
      console.log(`[compresso] ${sig} again — forcing exit`);
      process.exit(130);
    }
    shuttingDown = true;
